...
# Użycie Enum do bezpiecznego zarządzania gestami
class Gesture(Enum):
    OPEN_HAND = 'OPEN_HAND'
    POINTING = 'POINTING'
    VICTORY = 'VICTORY'
...
class CubeApp:
    ...
    def __init__(self, window: tk.Tk, window_title: str) -> None:
        ...
        self.last_action_gesture: Optional[str] = None
        ...
        # Lewa kolumna: Kamera i panel gestów
        ...
        # Prawa kolumna: Panel wizualizacji i obiekt 3D
        ...
        ttk.Button(
            info_frame, text="Resetuj Widok (gest 'Victory')", command=self._handle_view_reset
    ...
    def update(self) -> None:
        ...
        self.gesture_history.append(camera_output.gesture)
        if (
        ...
        if stable_gesture == Gesture.OPEN_HAND.value and camera_output.coords:
            ...
            self.target_angle_y = (
                (camera_output.coords[0] - 0.5) * base_angle_y * self.rotation_sensitivity
            )
            self.target_angle_x = (
                (camera_output.coords[1] - 0.5) * base_angle_x * self.rotation_sensitivity
            )

        if stable_gesture and stable_gesture != self.last_action_gesture:
            try:
                gesture_enum = Gesture(stable_gesture)
                if gesture_enum in self.gesture_actions:
                    self.gesture_actions[gesture_enum]()
            except ValueError:
                logging.debug(f"No action defined for gesture: {stable_gesture}")
